...
import { OneToOneLeadRequestBody } from "@/lib/types";
...
function validate(body: Partial<OneToOneLeadRequestBody>): string | null {
...
/**
 * Step-1 capture for the one-to-one booking. Mirrors /api/register-lead, but
 * writes to the `One-to-One Bookings` tab so an abandoned booking still leaves
 * a contactable lead behind.
 */
export async function POST(req: NextRequest) {
  let body: Partial<OneToOneLeadRequestBody>;
  try {
    body = await req.json();
  } catch {
  ...
  try {
    ...
    console.error("Apps Script one-to-one lead relay failed:", err);
